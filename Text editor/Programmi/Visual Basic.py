...
def open_file():
    file_path = filedialog.askopenfilename(filetypes=[("Visual Basic", "*.vb")])
    if file_path:
...
def save_file():
    file_path = filedialog.asksaveasfilename(defaultextension=".vb", filetypes=[("Visual Basic", "*.vb")])
    if file_path:
...
window = tk.Tk()
window.title("Dani 26 Visual Basic Editor")
