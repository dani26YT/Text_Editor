...
def open_file():
    file_path = filedialog.askopenfilename(filetypes=[("Ruby Files", "*.ruby")])
    if file_path:
...
def save_file():
    file_path = filedialog.asksaveasfilename(defaultextension=".ruby", filetypes=[("Ruby Files", "*.ruby")])
    if file_path:
...
window = tk.Tk()
window.title("Dani 26 Ruby Editor")
