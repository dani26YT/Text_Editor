...
def open_file():
    file_path = filedialog.askopenfilename(filetypes=[("PowerShell Files", "*.ps1")])
    if file_path:
...
def save_file():
    file_path = filedialog.asksaveasfilename(defaultextension=".ps1", filetypes=[("PowerShell Files", "*.ps1")])
    if file_path:
...
window = tk.Tk()
window.title("Dani 26 Powershell Editor")
