...
def open_file():
    file_path = filedialog.askopenfilename(filetypes=[("C# Files", "*.csharp")])
    if file_path:
...
def save_file():
    file_path = filedialog.asksaveasfilename(defaultextension=".csharp", filetypes=[("C# Files", "*.csharp")])
    if file_path:
...
window = tk.Tk()
window.title("Dani 26 C-Sharp Editor")
