...
def open_file():
    file_path = filedialog.askopenfilename(filetypes=[("F-Sharp Files", "*.fsharp")])
    if file_path:
...
def save_file():
    file_path = filedialog.asksaveasfilename(defaultextension=".fsharp", filetypes=[("F-Sharp Files", "*.fsharp")])
    if file_path:
...
window = tk.Tk()
window.title("Dani 26 f-sharp Editor")
